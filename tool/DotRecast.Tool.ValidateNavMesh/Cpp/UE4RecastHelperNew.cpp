...
size_t WriteTileData(std::FILE* fp, const dtMeshTile* tile, const dtNavMesh* mesh, bool cCompatibility)
{
    ...
    // 27. Write polygons - 完全按照 C# DtMeshDataWriter.WritePolys 的顺序
    for (int i = 0; i < h->polyCount; ++i)
    {
        const dtPoly* p = &tile->polys[i];
        // C#: if (cCompatibility) { RcIO.Write(stream, 0xFFFF, order); }
        if (cCompatibility)
        {
            WriteInt32LE(fp, 0xFFFF);
        }
        // C#: for (int j = 0; j < data.polys[i].verts.Length; j++) { RcIO.Write(stream, (short)data.polys[i].verts[j], order); }
        for (int j = 0; j < DT_VERTS_PER_POLYGON; ++j)
        {
            WriteInt16LE(fp, (int16_t)p->verts[j]);
        }
        // C#: for (int j = 0; j < data.polys[i].neis.Length; j++) { RcIO.Write(stream, (short)data.polys[i].neis[j], order); }
        for (int j = 0; j < DT_VERTS_PER_POLYGON; ++j)
        {
            WriteInt16LE(fp, (int16_t)p->neis[j]);
        }
        // C#: RcIO.Write(stream, (short)data.polys[i].flags, order);
        WriteInt16LE(fp, (int16_t)p->flags);
        // C#: RcIO.Write(stream, (byte)data.polys[i].vertCount);
        uint8_t polyVertCount = (uint8_t)p->vertCount;
        std::fwrite(&polyVertCount, sizeof(uint8_t), 1, fp);
        // C#: RcIO.Write(stream, (byte)data.polys[i].areaAndtype);
        uint8_t areaAndtype = (uint8_t)p->areaAndtype;
        std::fwrite(&areaAndtype, sizeof(uint8_t), 1, fp);
    }

    // 28. Write link placeholder - 完全按照 C# DtMeshDataWriter.Write 的逻辑
    // C#: if (cCompatibility) { byte[] linkPlaceholder = new byte[header.maxLinkCount * DtMeshDataReader.GetSizeofLink(false)]; stream.Write(linkPlaceholder); }
    // DtMeshDataReader.GetSizeofLink(false) = 16 (LINK_SIZEOF)
    if (cCompatibility)
    {
        uint8_t zero = 0;
    ...
    // 29. Write detail meshes - 完全按照 C# DtMeshDataWriter.WritePolyDetails 的顺序
    for (int i = 0; i < h->detailMeshCount; ++i)
    {
        const dtPolyDetail* dm = &tile->detailMeshes[i];
        // C#: RcIO.Write(stream, data.detailMeshes[i].vertBase, order);
        WriteInt32LE(fp, (int32_t)dm->vertBase);
        // C#: RcIO.Write(stream, data.detailMeshes[i].triBase, order);
        WriteInt32LE(fp, (int32_t)dm->triBase);
        // C#: RcIO.Write(stream, (byte)data.detailMeshes[i].vertCount);
        uint8_t detailMeshVertCount = (uint8_t)dm->vertCount;
        std::fwrite(&detailMeshVertCount, sizeof(uint8_t), 1, fp);
        // C#: RcIO.Write(stream, (byte)data.detailMeshes[i].triCount);
        uint8_t triCount = (uint8_t)dm->triCount;
        std::fwrite(&triCount, sizeof(uint8_t), 1, fp);
        // C#: if (cCompatibility) { RcIO.Write(stream, (short)0, order); }
        if (cCompatibility)
        {
            WriteInt16LE(fp, 0);
        }
    ...
    // 30. Write detail vertices - 完全按照 C# DtMeshDataWriter.WriteVerts 的逻辑
    // C#: WriteVerts(stream, data.detailVerts, header.detailVertCount, order);
    // C#: for (int i = 0; i < count * 3; i++) { RcIO.Write(stream, verts[i], order); }
    for (int i = 0; i < h->detailVertCount * 3; ++i)
    {
        WriteFloatLE(fp, tile->detailVerts[i]);
    }

    // 31. Write detail triangles - 完全按照 C# DtMeshDataWriter.WriteDTris 的逻辑
    // C#: for (int i = 0; i < data.header.detailTriCount * 4; i++) { RcIO.Write(stream, (byte)data.detailTris[i]); }
    for (int i = 0; i < h->detailTriCount * 4; ++i)
    {
        uint8_t triByte = (uint8_t)tile->detailTris[i];
        std::fwrite(&triByte, sizeof(uint8_t), 1, fp);
    }

    // 32. Write BV tree - 完全按照 C# DtMeshDataWriter.WriteBVTree 的顺序
    for (int i = 0; i < h->bvNodeCount; ++i)
    {
        const dtBVNode* node = &tile->bvTree[i];
        // C#: if (cCompatibility) { RcIO.Write(stream, (short)data.bvTree[i].bmin.X, order); ... }
        if (cCompatibility)
        {
        ...
        // C#: else { RcIO.Write(stream, data.bvTree[i].bmin.X, order); ... }
        else
        {
        ...
        // C#: RcIO.Write(stream, data.bvTree[i].i, order);
        WriteInt32LE(fp, node->i);
    }

    // 33. Write off-mesh connections - 完全按照 C# DtMeshDataWriter.WriteOffMeshCons 的顺序
    for (int i = 0; i < h->offMeshConCount; ++i)
    {
        const dtOffMeshConnection* con = &tile->offMeshCons[i];
        // C#: for (int j = 0; j < 2; j++) { RcIO.Write(stream, data.offMeshCons[i].pos[j].X, order); RcIO.Write(stream, data.offMeshCons[i].pos[j].Y, order); RcIO.Write(stream, data.offMeshCons[i].pos[j].Z, order); }
        // C# 中 pos 是 RcVec3f[2]，即 6 个 float: [ax, ay, az, bx, by, bz]
        // C++ 中 pos 是 dtReal pos[6]，顺序相同
        for (int j = 0; j < 2; ++j)
        {
            WriteFloatLE(fp, con->pos[j * 3 + 0]); // X (ax/bx)
            WriteFloatLE(fp, con->pos[j * 3 + 1]); // Y (ay/by)
            WriteFloatLE(fp, con->pos[j * 3 + 2]); // Z (az/bz)
        }
        // C#: RcIO.Write(stream, data.offMeshCons[i].rad, order);
        WriteFloatLE(fp, con->rad);
        // C#: RcIO.Write(stream, (short)data.offMeshCons[i].poly, order);
        WriteInt16LE(fp, (int16_t)con->poly);
        // C#: RcIO.Write(stream, (byte)data.offMeshCons[i].flags);
        uint8_t flags = (uint8_t)con->flags;
        std::fwrite(&flags, sizeof(uint8_t), 1, fp);
        // C#: RcIO.Write(stream, (byte)data.offMeshCons[i].side);
        uint8_t side = (uint8_t)con->side;
        std::fwrite(&side, sizeof(uint8_t), 1, fp);
        // C#: RcIO.Write(stream, data.offMeshCons[i].userId, order);
